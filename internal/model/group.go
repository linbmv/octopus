...
const (
	...
	GroupModeRoundRobin GroupMode = "round_robin" // 按顺序轮换成员。
	GroupModeRandom     GroupMode = "random"      // 随机选择成员。
	GroupModeWeighted   GroupMode = "weighted"    // 按成员权重随机选择。
)
...
type Group struct {
	...
	Mode         GroupMode        `json:"mode" gorm:"not null;default:manual" binding:"omitempty,oneof=manual failover round_robin random weighted"` // 选择成员的模式。
...
type GroupItem struct {
	...
	Weight         int           `json:"weight" gorm:"not null;default:1"`                                                                   // Weighted 模式下的相对权重。
...
type GroupUpdateRequest struct {
	...
	Mode          *GroupMode               `json:"mode,omitempty" binding:"omitempty,oneof=manual failover round_robin random weighted"` // Mode 仅在选择模式变更时发送。
...
type GroupItemAddRequest struct {
	...
	Weight         int           `json:"weight,omitempty"`                                             // Weighted 模式下的相对权重。
}
...
type GroupItemUpdateRequest struct {
	...
	Weight   int   `json:"weight,omitempty"`      // Weighted 模式下的新权重，0 表示不变。
