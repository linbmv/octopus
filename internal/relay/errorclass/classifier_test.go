...
func TestClassifyKeyLevelRetryDecision(t *testing.T) {
	tests := []struct {
		name         string
		statusCode   int
		responseBody []byte
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := Classify(tt.statusCode, tt.responseBody).Level == ErrorLevelKey
			if got != tt.want {
				t.Errorf("Classify(%d, %q) key-level = %v, want %v",
