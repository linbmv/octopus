...
func TestNewIteratorWithCandidateRanksPrioritizesCapabilityBeforeGroupPriority(t *testing.T) {
	clearSessions()
	defer clearSessions()
	...
	group := model.Group{
		Mode: model.GroupModeFailover,
		Items: []model.GroupItem{
			{ID: 101, ChannelID: 1, ModelName: "chat-manual", Priority: 1},
			{ID: 102, ChannelID: 2, ModelName: "official", Priority: 99},
			{ID: 103, ChannelID: 3, ModelName: "responses-manual", Priority: 2},
		},
	}
	ranks := map[int]int{
		101: 2,
	...
	iter := NewIteratorWithCandidateRanks(group, 1, "octopus-model", ranks)
	want := []int{2, 3, 1}
	for _, channelID := range want {
		if !iter.Next() {
			t.Fatalf("Next() = false, want channel %d", channelID)
		}
		if got := iter.Item().ChannelID; got != channelID {
			t.Fatalf("channel = %d, want %d", got, channelID)
		}
	}
}

func TestNewIteratorWithCandidateRanksDoesNotMoveLowerRankStickyAhead(t *testing.T) {
	clearSessions()
	defer clearSessions()
	...
	SetSticky(apiKeyID, requestModel, 1, 101, "chat-manual")
	group := model.Group{
		Mode:            model.GroupModeFailover,
		SessionKeepTime: 60,
		Items: []model.GroupItem{
	...
	iter := NewIteratorWithCandidateRanks(group, apiKeyID, requestModel, ranks)
	if !iter.Next() {
		t.Fatal("Next() = false, want true")
	}
	if got := iter.Item().ChannelID; got != 2 {
		t.Fatalf("first channel = %d, want official channel 2", got)
	}
	if iter.IsSticky() {
		t.Fatal("lower-rank sticky candidate should not be marked sticky before higher-rank candidate")
	}
}
