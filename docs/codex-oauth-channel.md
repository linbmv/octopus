...
- 已登录的管理界面顶部会显示 **Codex 全局额度**，按渠道、凭据备注和套餐汇总所有 Codex
  OAuth 渠道的 5 小时、7 天、Code Review 及附加额度窗口；顶部的 **刷新全部** 会绕过缓存
 重新查询，渠道详情仍保留逐凭据的完整明细。全局接口同样不返回账号邮箱、账户 ID 或 OAuth token，
  单次查询最多等待 20 秒并允许不同渠道并行查询。
