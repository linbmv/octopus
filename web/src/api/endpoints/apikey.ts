...
export function useAPIKeyDashboardStats() {
    ...
    return useQuery({
        ...
        select: (data): APIKeyStatsResponseFormatted => ({
            stats: { ...formatStatsMetrics(data.stats), api_key_id: data.stats.api_key_id },
            info: data.info,
        }),
...
export function useAPIKeyStats() {
    return useQuery({
        ...
        select: (data): StatsAPIKeyFormatted => ({ ...formatStatsMetrics(data), api_key_id: data.api_key_id }),
        refetchInterval: 30000,
